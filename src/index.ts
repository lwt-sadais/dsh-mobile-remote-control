...
import { isTrustedApiRequest, makeRoutes, PAIR_PATHS } from './routes.ts'
...
export const name = 'dsh-mobile-remote-control'
...
export interface Config {
  ...
  /** Dedicated least-privilege LAN listener for the mobile-only facade. */
  mobileLanPort?: number
...
export const Config: z<Config> = z.object({
  ...
  mobileLanPort: z.number().step(1).min(1).max(65535).default(3081),
 ...
 */
export const apply = mountOnce('dsh-mobile-remote-control', applyImpl)
...
function applyImpl(ctx: Context, config?: Config): void {
  const resolved: ResolvedConfig = {
    ...
    mobileLanPort: config?.mobileLanPort ?? DEFAULTS.mobileLanPort,
  ...
  const resolve = (): ResolvedConfig => {
    ...
    return {
      ...
      mobileLanPort: value.mobileLanPort ?? DEFAULTS.mobileLanPort,
  ...
  const mobileLanPort = resolved.mobileLanPort
  const lanBases = lanIPv4Addresses().map(address => ({ address, base: `http://${address}:${String(mobileLanPort)}` }))
  ...
  // rc.7 intentionally keeps the main DSH server loopback-only. Expose only
  // the standalone phone surface, its allowlisted BFF, and phone-side pairing
  // endpoints on a separate LAN listener; ordinary /api and desktop controls
  // are absent from this listener.
  const lanRoutePaths = new Set([
    ...
    PAIR_PATHS.accept, PAIR_PATHS.heartbeat, PAIR_PATHS.status, PAIR_PATHS.events,
  ])
  const lanRoutes = routes.filter(route =>
    (route.kind === 'exact' && lanRoutePaths.has(route.path))
    || (route.kind === 'prefix' && route.path === '/m/api'),
  )
  const lanServer = createServer((request, response) => {
    const pathname = new URL(request.url ?? '/', 'http://mobile.local').pathname
    const route = lanRoutes.find(candidate => candidate.kind === 'exact' && candidate.path === pathname)
      ?? lanRoutes.filter(candidate => candidate.kind === 'prefix' && pathname.startsWith(candidate.path))
    ...
    Promise.resolve(route.handler(request, response)).catch(() => {
      if (!response.headersSent) response.writeHead(400, { 'cache-control': 'no-store' })
      response.end()
    })
  })
  lanServer.listen(mobileLanPort, '0.0.0.0')
  ctx.effect(() => () => new Promise<void>(resolveClose => lanServer.close(() => resolveClose())), 'mobile-remote: least-privilege LAN listener')
  ...
  const sync = (): void => {
    ...
    if (autoTunnel) {
      ...
      tunnel.start(`http://127.0.0.1:${String(mobileLanPort)}`)
    } else {
      tunnel.stop()
