...
//#region ../../node_modules/.pnpm/@deepseek-ai+dsh-settings@0.1.0-rc.8_@deepseek-ai+cordis@4.0.1_@deepseek-ai+dsh-brand@0_7616943ecf171cb97db9e2088de3356f/node_modules/@deepseek-ai/dsh-settings/lib/index.js
/**
...
function makeMobileRoutes() {
	...
	const handleBundle = async (_req, res) => {
		if (bundleBody === void 0) {
			...
			if (!existsSync(path)) {
				writeStatic(res, 503, "text/plain", "mobile bundle not built: run pnpm --filter dsh-mobile-remote-control build");
				return;
			}
			try {
...
const SELF_PACKAGE = "dsh-mobile-remote-control";
...
function familyUpdatePackages(anchor, anchorManifest, profileManifest) {
	...
	for (const [name, spec] of Object.entries(dependencies)) if (name.startsWith("@linxin666/") && typeof spec === "string" && !isLinkedSpec(spec)) names.add(name);
	return [...names];
}
/**
...
function readInstalledVersion(resolve, name, profileDir) {
	...
	if (profileDir === void 0 || !/^@linxin666\/[a-z0-9][a-z0-9._-]*$/.test(name)) return VERSION_UNKNOWN;
...
const name = "dsh-mobile-remote-control";
...
*/
const apply = mountOnce("dsh-mobile-remote-control", applyImpl);
function applyImpl(ctx, config) {
	...
	const mobileLanPort = 3081;
	const lanBases = lanIPv4Addresses().map((address) => ({
		address,
		base: `http://${address}:${String(mobileLanPort)}`
	}));
	service.setLanBases(lanBases);
	const mobileLanExact = /* @__PURE__ */ new Set([
	...
	const mobileLanServer = http.createServer((incoming, outgoing) => {
		const pathname = new URL(incoming.url ?? "/", "http://mobile.local").pathname;
		if (!mobileLanExact.has(pathname) && !pathname.startsWith("/m/api/")) {
			outgoing.writeHead(404, { "cache-control": "no-store", "x-content-type-options": "nosniff" });
			outgoing.end();
			return;
		}
		const upstream = request({
	...
	mobileLanServer.on("error", (error) => ctx.logger.error(error));
	mobileLanServer.listen(mobileLanPort, "0.0.0.0");
	ctx.effect(() => () => new Promise((resolveClose) => mobileLanServer.close(() => resolveClose())), "mobile-remote: least-privilege LAN listener");
