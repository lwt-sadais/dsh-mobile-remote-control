...
### From a regular system terminal

A regular terminal must explicitly target the DSH Desktop `desktop` profile:

```sh
dsh plugin --profile desktop github:lwt-sadais/dsh-mobile-remote-control
```

### From the DSH Desktop terminal

The terminal opened inside DSH Desktop already uses the `desktop` profile, so omit `--profile desktop`:

```sh
dsh plugin github:lwt-sadais/dsh-mobile-remote-control
```

Restart DSH Desktop or the corresponding DSH Web Host after installation.
...
```sh
git clone https://github.com/lwt-sadais/dsh-mobile-remote-control.git
cd dsh-mobile-remote-control
pnpm install
dsh plugin --profile desktop link:$(pwd)
```
